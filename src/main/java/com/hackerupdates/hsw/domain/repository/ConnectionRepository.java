...
public interface ConnectionRepository extends JpaRepository<Connection, Long> {
    ...
    @Query("SELECT new com.hackerupdates.hsw.domain.dto.ConnectionDTO(c.connectionId, c.pinned) "
    ...
    @Query("SELECT new com.hackerupdates.hsw.domain.dto.PersonSumDTO(p.id, p.userName, p.name)  FROM Connection c LEFT JOIN Person p ON c.personId = p.id"
        + " WHERE p.status = :status AND c.blocked = false AND c.connectionId = :personId")
    List<PersonSumDTO> findNameByConnectionId(Long personId, Status status);

    @Query("SELECT new com.hackerupdates.hsw.domain.dto.PersonSumDTO(p.id, p.userName, p.name) FROM Connection c LEFT JOIN Person p ON c.connectionId = p.id"
